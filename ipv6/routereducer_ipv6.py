...
def sort_subnets(subnets):
    # Correct the subnets to ensure they align with their prefix length.
    corrected_subnets = [ip_network(subnet, strict=False) for subnet in subnets]
    # Sort corrected subnets based on the network address and prefix length
    corrected_subnets.sort(key=lambda net: (net.network_address, net.prefixlen))
    return [str(net) for net in corrected_subnets]

# Load the subnets from 'subnets.txt'
with open('subnets.txt') as file:
    subnets = [line.strip() for line in file if line.strip()]

# Sort and correct the subnets
sorted_corrected_subnets = sort_subnets(subnets)

# Prepare the input for aggregate6, with corrected subnets as strings
subnets_str = "\n".join(sorted_corrected_subnets)

# Run aggregate6 with the '-6' flag for IPv6 addresses
result = subprocess.run(['aggregate6', '-6'], input=subnets_str, text=True, capture_output=True)
...
# Convert corrected subnets back to ip_network objects for comparison
original_networks = [ip_network(subnet) for subnet in sorted_corrected_subnets]
...
with open(csv_file_path, 'w', newline='') as csvfile:
    ...
    for summarized_subnet in summarized_subnets:
        ...
        summarized_network = ip_network(summarized_subnet, strict=False)
        ...
        if summarized_network.prefixlen == 128:
            comments = "Host Address"
        elif len(contributing_subnets) == 1 and contributing_subnets[0] == str(summarized_network):
